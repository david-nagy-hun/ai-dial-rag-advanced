...
class TextProcessor:
    ...
    def process_text_file(self, file_name: str, chunk_size: int, chunk_overlap: int, dimensions: int, truncate_table: bool = True):
        """
        ...
        if chunk_overlap >= chunk_size:
            raise ValueError("chunk_overlap should be lower than chunk_size")
        ...
        chunks = chunk_text(text=file_content, chunk_size=chunk_size, overlap=chunk_overlap)
        ...
        for i in range(len(chunks)):
            self._save_chunk(file_name, chunks[i], embeddings.get(i))
    ...
    def _save_chunk(self, document_name: str, chunk: str, embedding: list[float]):
        """Save chunk with embedding to database"""
        vector_string = f"[{','.join(map(str, embedding))}]"
        save_query = f"INSERT INTO vectors (document_name, text, embedding) VALUES (%s, %s, %s::vector);"

        with self._get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute(save_query, (document_name, chunk, vector_string))
                conn.commit()
    ...
    def search(self, search_mode: SearchMode, user_request: str, top_k: int, score_threshold: float, dimensions: int) -> list[str]:
        ...
        embedding = self.embeddings_client.get_embeddings(user_request, dimensions)[0]
        vector_string = f"[{','.join(map(str, embedding))}]"
        ...
        with self._get_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                search_query = self._get_search_query(search_mode)
                cursor.execute(query=search_query, vars=(vector_string, vector_string, max_distance, top_k))
    ...
    @staticmethod
    def _get_search_query(search_mode: SearchMode) -> str:
