...
completion_client = DialChatCompletionClient("gpt-4o", API_KEY)
...
def main():
    ...
    if load_context.lower().strip() in ['y', 'yes']:
        text_processor.process_text_file("task/embeddings/microwave_manual.txt", 150, 40, 1536)
        print("="*100)
    ...
    while True:
        ...
        # Step 1: Retrieval
        print(f"{'=' * 100}\n🔍 STEP 1: RETRIEVAL\n{'-' * 100}")
        context_parts = text_processor.search(SearchMode.EUCLIDIAN_DISTANCE, user_request, 5, 0.5, 1536)
        context = "\n\n".join(context_parts)

        # Step 2: Augmentation
        print(f"\n{'=' * 100}\n🔗 STEP 2: AUGMENTATION\n{'-' * 100}")
        augmented_prompt = USER_PROMPT.format(query=user_request, context=context)
